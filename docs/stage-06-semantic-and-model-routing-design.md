...
- exact manifest-digest consent and invalidation;
- an ephemeral signed-in Codex Desktop adapter that rejects observed tool use;
- a deterministic pair/dimension question plan with explicit truncation;
- two isolated calls: a primary analyst and a reversed-order adversarial critic;
- closed cited-response and bounded recommendation contracts;
- local semantic adjudication into the same sealed result graph.

The local adapter is opt-in and developmental. No provider/model/adapter/prompt
identity has completed the Stage 04 qualification protocol, so a successful run
is not an accuracy, calibration, usefulness, or release-readiness claim. The
path uses the signed-in Codex Desktop account and does not require an OpenAI API
key; it does not prove availability to a separate API project.
...
an exact disclosure digest. Untrusted repository text cannot grant consent or
trigger provider use.
...
corresponding consent/cache/qualification identity as required by Stage 03–04.
...
1. Complete frozen scope, inventory, parsing, reference/configuration/
   ...
   checks.
2. Build one semantic question for one claim set, region, and dimension only
   where semantic evidence can add value.
...
6. Display the manifest and require affirmative consent bound to its digest.
   Scan consent, model consent, and write authority remain non-substitutable.
7. Invoke a bounded analyst in a fresh ephemeral Codex context with no direct
   filesystem access and no credentials in the payload or logs. It must answer
   every frozen pair/dimension question exactly once.
8. Invoke an independent critic in a second fresh context. Present source
   handles in reversed order and require it to attempt to refute every analyst
   answer, including recommendation risk and missing evidence.
9. Validate both transport lifecycles, closed schemas/labels/recommendations,
...
10. Record both model statements only as immutable `inferred` evidence.
11. Let the local adjudicator require critic corroboration, closed
    counterexamples, no missing evidence, and a recommendation/label
...
The analyst may return one candidate relation per frozen question, cited
...
recommendation candidate. The critic may corroborate, challenge, or declare
the answer insufficient. Neither call may set product check state, severity,
final confidence, authority, repair operations, scope, or evidence provenance.
Model agreement never becomes deterministic proof.
...
| Consent digest absent or mismatched | `not_run`; zero provider requests |
...
| 06-B | Production disclosure manifest and exact per-run consent | 06-A | Secret/script exclusion, digest invalidation, and zero-call mismatch tests pass | Implemented locally |
| 06-C | Two-pass Codex Desktop panel, schema/citation/recommendation validation, local adjudication bridge, human and sealed projections | 06-B | Panel identity, disagreement, authority/citation gates, human-output and sealed-graph integration tests pass | Implemented, unqualified |
...
- model/effort/profile/provider/content/purpose changes invalidate consent;
...
- analyst/critic identity mismatches, missing reviews, and uncited reviews are
  rejected;
- critic disagreement abstains instead of being hidden by majority voting;
- critic source order is reversed and prompt-injection text remains quoted data;
...
6. Product semantic invocation requires disclosure consent, minimization,
