...
13. 已完成但仍属 `insufficient_evidence` 的面板假设必须进入独立的“未确认语义线索”区，
    不能混入 finding/candidate。终端展示最像风险的前三项及最明确的无实质关系/互补后两项，
    包含精确原句与行号、各角色标签和理由、开放反例、稳定 ID 及精确剩余数量；Markdown/JSON
    保留全部线索。该展示投影不得改变结果图状态或提升任何假设。
