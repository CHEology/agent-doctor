...
- 排除秘密与脚本的确定性语义披露代理；
- 与 manifest digest 精确绑定的同意及失效规则；
- 使用已登录 Codex Desktop、拒绝任何已观察工具调用的 ephemeral 适配器；
- 确定性的 Skill 对/维度问题计划，并显式记录截断；
- 两次隔离调用：主分析者与逆序读取来源的对抗性批评者；
- 封闭且强制引用的响应契约与有边界的建议契约；
- 写入同一封存结果图的本地语义裁决。

本地适配器必须显式 opt-in，且属于开发性路径。没有任何 provider/model/adapter/prompt
身份完成 Stage 04 资格协议，因此成功运行不代表准确率、校准、有用性或发布就绪。它
使用已登录的 Codex Desktop 账号，不需要 OpenAI API key；也不能据此推断另一个 API
项目可用该模型。
...
| `semantic.reasoning_quality_first` | `gpt-5.6-sol` | `max` | 已同意的开发性 Codex Desktop 适配器；尚未资格验证 |
...
契约、分类法、输入或政策任一变化，都会按 Stage 03–04 要求使相应 consent/cache/
qualification 身份失效。
...
1. 先完成冻结范围、盘点、解析、引用/配置/优先级/适用性解析，以及所有相互独立的
   确定性检查；
2. 只对语义证据确实有价值的问题，按一个主张集合、一个区域、一个维度建立一个语义
   问题；
...
6. 展示清单，并要求绑定其摘要的主动同意。扫描同意、模型同意和写授权互不替代；
7. 在全新的 ephemeral Codex 上下文中调用有边界的分析者。它没有任意文件系统访问权，
   payload 与日志不含凭证，并且必须逐一回答每个冻结的 Skill 对/维度问题；
8. 在第二个全新上下文中调用独立批评者。把来源句柄逆序呈现，要求它主动反驳每个分析
   答案，并检查建议风险与缺失证据；
9. 校验两次调用的生命周期、封闭 schema/标签/建议、问题/来源/维度的精确 join、内容
   句柄引用、秘密回显、来源顺序披露和请求/响应身份；非法或无引用面板输出不可使用；
10. 两个模型角色的陈述都只能作为不可变 `inferred` 证据；
11. 本地裁决器必须先确认批评者同意、反例已关闭、没有缺失证据、建议与标签兼容，再负责
    适用性、分类法、状态、标签、限定符、严重度、置信度、去重和分组；
12. 最终仍封存一份供人类可读终端、Markdown、JSON 和 CI 共用的结果图。

### 6.2 提供商响应契约

分析者可以针对每个冻结问题返回一个候选关系、已披露句柄/主张引用、简短理由、共同区域
判断、各自贡献、反例状态、缺失证据，以及一个有边界的手工建议候选。批评者只能选择
“支持、挑战、证据不足”。两个角色都不得设置产品检查状态、严重度、最终置信度、权限、
修复操作、范围或证据来源类型；即便双方一致，也不能把模型意见升级为确定性证明。
...
| 同意摘要不存在或不匹配 | `not_run`，provider 请求数为零 |
...
| 06-B | 生产 disclosure manifest 与逐次精确同意 | 06-A | 秘密/脚本排除、摘要失效、错摘要零调用测试通过 | 本地已实现 |
| 06-C | 双轮 Codex Desktop 面板、schema/引用/建议校验、本地裁决桥、人类输出与封存图接入 | 06-B | 面板身份、分歧、权限/引用门禁、人类输出与封存图测试通过 | 已实现，未资格验证 |
...
- 一个披露句柄不能泄漏另一来源；
- model/effort/profile/provider/content/purpose 变化使 consent 失效；
- 每个被接受引用都指向已披露句柄及精确版本；
- 在请求、日志、报告、指纹、缓存和失败包中都检查秘密/脚本排除；
- 拒绝模型返回的 state/severity/authorization；
- 分析者/批评者身份不一致、缺少批评或批评引用缺失必须被拒绝；
- 批评者有分歧时必须弃权，不能用多数票掩盖；
- 批评者必须逆序读取来源，prompt injection 文字始终只是被引用的数据；
...
6. 产品语义调用必须具备披露同意、最小化、适配器校验、推断来源、本地裁决和资格验证；
